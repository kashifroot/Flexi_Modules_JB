odoo.define('flexiretail_advance.models', function (require) {
    "use strict";
    ...
    models.load_fields("res.users", ['image_small', 'login', 'company_ids', 'access_ereceipt', 'access_quick_cash_payment',
        ...
        'discard_product', 'based_on', 'can_give_discount', 'can_update_price', 'discount_limit', 'login_with_pos_screen', 'store_ids', 'access_pos_dashboard',
    ...
    models.load_fields("res.company", ['payment_total', 'pos_price', 'pos_quantity', 'pos_discount', 'pos_search', 'pos_next', 'street_name', 'city', 'state_id', 'zip', 'country_id']);
    ...
    models.PosModel.prototype.models.push({
        ...
        model: 'quantity.discount.fixed',
        fields: [],
        loaded: function (self, pos_quantity_fixed_ids) {
            self.pos_quantity_fixed_ids = pos_quantity_fixed_ids;
        },
    }, {
    ...
    function decimalAdjust(value) {
        ...
        //	        nagative_sign = true;
        //	    }
        //	    if(.contains(.range(0,5), reminder_value)){
        //	        rounding_value = eval(split_value[0].toString() + '.' + division_value.toString() + '0' )
        //	    }else if(.contains(.range(5,10), reminder_value)){
        ...
        var rounding_value = Math.round(float_value * 20) / 20;
        return rounding_value
    }
    ...
    models.PosModel = models.PosModel.extend({
        ...
        load_new_products_price: function () {
            var self = this;
            var def = new $.Deferred();
            var currency_symbol = self.currency ? self.currency : { symbol: '$', position: 'after', rounding: 0.01, decimals: 2 };

            // Fetch only product id, name, and price
            var fields = ['id', 'name', 'list_price', 'standard_price'];

            // Define the product domain and conditions for updated records
            var pos_domain = this.product_domain;
            var prod_domain = [['write_date', '>', self.db.get_product_write_date()]];
            prod_domain = prod_domain.concat(pos_domain);
            ...
            rpc.query({
                model: 'product.product',
                method: 'load_latest_product_price',
                args: [write_date, fields, context]
            }, {
                timeout: 3000,
                shadow: true,
                ...
                if (res && res.variants) {
                    _.each(res.variants, function (product) {
                        var existing_product = self.db.get_product_by_id(product.id);

                        // Only update name and price if product exists
                        if (existing_product) {
                            existing_product.display_name = product.name;
                            existing_product.list_price = product.list_price;
                            existing_product.standard_price = product.standard_price;
                            ...
                            if (current_pricelist) {
                                prod_obj.product_cache.clear_node(product.id + ',' + current_pricelist.id);
                                prod_obj.render_product(existing_product);
                            }
                        }
                    });

                    // Re-render product categories if needed
                    self.gui.screen_instances.products.product_categories_widget.renderElement();
                }
            });
            return def;
        ...
        get_cashier: function () {
            return this.user || this.get('cashier') || this.db.get_cashier();
        },
        set_cashier: function (user) {
            ...
            if (this.user.pos_user_type == "cashier") {
                var button_clock = QWeb.render('SaleNoteIconChrome', { widget: self, user: this.user });
                var draft_orders = []
                ...
                //				$('.sale_note_icon_widget').css('display','none');
            }
            var params = {
                model: 'pos.session',
                method: 'write',
...
//            }, 60 * 1000);
        ...
        add_new_order: function () {
            ...
            if (order_list.length == 0) {
                this.pos_session.sequence_number = 1
            }
    ...
    models.Order = models.Order.extend({
        initialize: function (attributes, options) {
            ...
            $('.order-information').addClass('oe_hidden');
        ...
        // },
        //		Yulia 13112024 set discount every orderline
        set_discount_total: function (orderlines, discount_amt, mode) {
            var disc_input = Math.min(Math.max(parseFloat(discount_amt) || 0, 0), 100);
            let amt = orderlines.reduce((total, line) => {
                let price_amount = line.get_price_with_tax();
                return total + Number(price_amount);
            }, 0);

            let disc = (discount_amt / amt) * 100
            //kashif round_di(amount,decimals).toFixed(decimals);
            let after_disc = orderlines.reduce((total, line) => {
                let price_amount = line.get_price_with_tax();
                let discountedAmount = round_pr(price_amount * disc / 100, 0.1).toFixed(2);
                line.set_discount_amount(discountedAmount, mode);
                line.trigger('change', line);
                return total + discountedAmount;
            }, 0);

            let parsed_disc = Math.min(Math.max(parseFloat(after_disc) || 0, 0), 100)
            this.discount_amount_total = parsed_disc;
            this.before_disc_total = disc;
            this.discount = disc;
            this.discountStr = '' + disc;
            //             this.trigger('change',this);
        },
        //             kasihf 22jan25: added reset button in pos functionality

        reset_orderlines: function () {
            var self = this;
            var new_orders = _.sortBy(self.get_orderlines(), 'cid').reverse();
            new_orders.map(function (item) {
                item.set_quantity('remove');
                self.trigger('change', self);
                self.add_product(item.product, {
                    quantity: 1,
        ...
        set_discount_total_percent: function (orderlines, discount_amt, mode) {
            var disc_input = Math.min(Math.max(parseFloat(discount_amt) || 0, 0), 100);
            let amt = orderlines.reduce((total, line) => {
                let price_amount = line.get_price_with_tax();
                return total + Number(price_amount);
        ...
        get_before_disc_total: function () {
            return this.before_disc_total
        },
        ...
        change_mode: function (mode) {
            ...
            if (mode == 'sale') {
                if (this.attributes.draft_order) {
                    $('.order-information').removeClass('oe_hidden');
                }
                else {
                    $('.order-information').addClass('oe_hidden');
                }
        ...
        add_product: function (product, options) {
            ...
            lines.forEach(function (line) {
                if (line.product === product) {
                    self.select_orderline(line);
                }
            });
            if (!$('.order').hasClass('collasped_unsubwindow') && !$('.order').hasClass('collasped_subwindow')) {
        ...
        apply_promotion: function () {
            ...
            var pos_get_qty_fixed_list = self.pos.pos_quantity_fixed_ids;
                    ...
                    if (promotion_list && promotion_list[0]) {
                        _.each(promotion_list, function (promotion) {
                            if ((Number(promotion.from_time) <= current_time && Number(promotion.to_time) > current_time) || (!promotion.from_time && !promotion.to_time)) {
                                ...
                                } else if (promotion && promotion.promotion_type == "quantity_discount_fixed") {
                                    if (promotion.product_id_amt && promotion.product_id_amt[0] == line.product.id) {
                                        var line_ids = [];
                                        var apply_promotion = false;
                                        _.each(promotion.pos_quantity_fixed_ids, function (pos_quantity_fixed_id) {
                                            var line_record = _.find(pos_get_qty_fixed_list, function (obj) { return obj.id == pos_quantity_fixed_id });
                                            if (line_record) {
                                                if (line_record.operator == 'equal') {
                                                    if (line.get_quantity() == line_record.quantity) {
                                                        apply_promotion = true;
                                                    }
                                                }
                                                else if (line_record.operator == 'equal_or_greater') {
                                                    if (line.get_quantity() >= line_record.quantity) {
                                                        apply_promotion = true;
                                                    }
                                                }
                                                else if (line_record.operator == 'greater') {
                                                    if (line.get_quantity() > line_record.quantity) {
                                                        apply_promotion = true;
                                                    }
                                                }
                                                if (apply_promotion == true) {
                                                    apply_promotion = false;
                                                    if (line_record.discount) {
                                                        line.set_discount_amt(line_record.discount);
                                                        line.set_discount_amt_rule(promotion.promotion_code);
                                                        line.set_promotion_data(promotion);
                                                        line.set_unit_price(((line.get_unit_price() * line.get_quantity()) - line_record.discount) / line.get_quantity());
        ...
        set_user_name: function (user_id) {
            this.set('user_id', this.user || user_id);
        },
        ...
        export_as_JSON: function () {
            ...
            var new_val = {
                ...
                is_draft_order: this.is_draft_order || false,
        ...
        export_for_printing: function () {
            ...
            var new_val = {
                ...
                //            yulia
                discount_amount_total: this.get_discount_amount_total(),
                before_disc_total: this.get_before_disc_total()
            };
            $.extend(orders, new_val);
            return orders;
        },
        ...
        set_sticker: function (res) {
            this.sticker = res;
        },
        get_sticker: function () {
            return this.sticker;
        },
        //        Yulia 21012025 set and get function for tin, brn, tour code
        set_tin: function (res) {
            this.tin = res
        },
        get_tin: function () {
            return this.tin
        },
        set_brn: function (res) {
            this.brn = res
        },
        get_brn: function () {
            return this.brn
        },
        set_tour_code: function (res) {
            this.tour_code = res
        },
        get_tour_code: function (res) {
    ...
    models.Orderline = models.Orderline.extend({
        initialize: function (attr, options) {
            ...
            this.discount_amount_str = '0';
            this.manual_discount = false;
            this.second_name = false;
            this.all_discounts_applied = [];
            this.first_discount_amount = 0.00
        },
        ...
        set_discount: function (discount, mode = "discount") {
            var disc = Math.min(Math.max(parseFloat(discount) || 0, 0), 100);
            var existing_discount = 0;
            if (discount !== "" & discount !== "0.00") {
                if (this.all_discounts_applied.includes(mode) & this.all_discounts_applied.length < 3) {
                    console.log("all_discounts_applied");
                    existing_discount = parseFloat(this.first_discount_amount.toFixed(2));

                    //              existing_discount = (this.price * this.quantity) - this.get_price_without_tax();

                }
                else if (this.all_discounts_applied.length >= 2) {
                    this.do_warn(_t("Error"), _t("Error,Cannot apply more than 2 discounts. kindly remove or reset the order lines"));
                    //                throw new Error("Cannot apply more than 2 discounts. kindly remove or reset the order lines");
                }
                else {
                    this.all_discounts_applied.push(mode);
                    existing_discount = parseFloat(((this.price * this.quantity) - this.get_price_without_tax()).toFixed(2));
                    this.first_discount_amount = existing_discount;
            ...
            this.discount = parseFloat(disc + existing_discount).toFixed(2);
            this.discountStr = '' + this.discount;
            this.set_manual_discount(false);
            this.trigger('change', this);
        },
        ...
        get_base_price: function () {
            ...
            if (this.get_manual_discount()) {
                return round_pr(round_pr(this.get_unit_price() * this.get_quantity(), rounding) - round_pr(this.get_discount_amount(), rounding), rounding);
                //        return round_pr(this.get_unit_price() * this.get_quantity() * (1 - this.get_discount_amount()/100), rounding);
            }
            else {
        ...
        get_all_prices: function () {
            ...
            var qty = 1;
            if (this.get_manual_discount()) {
                var price_unit = round_pr(round_pr(this.get_unit_price() * this.get_quantity(), rounding) - round_pr(this.get_discount_amount(), rounding), rounding);

                //               var price_unit = round_pr((this.get_unit_price()) - round_pr(this.get_discount_amount(),rounding),rounding);
                //            var price_unit = this.get_unit_price() * (1.0 - (this.get_discount_amount() / 100.0));
            }
            else {
        ...
        //        discount amount kashif

        set_manual_discount: function (val) {
            this.manual_discount = val;
        },
        //         set_discount_amount: function(discount_amt){
        //
        //          this.trigger('change',this);
        //         },
        set_discount_amount: function (discount_amt, mode) {
            var disc = Math.min(Math.max(parseFloat(discount_amt) || 0, 0), 100);
            var existing_discount = 0;

            if (discount_amt !== "" & discount_amt !== "0.00") {
                if (this.all_discounts_applied.includes(mode) & this.all_discounts_applied.length < 3) {
                    //                 existing_discount = (this.price * this.quantity) - this.get_price_without_tax();
                    existing_discount = parseFloat(this.first_discount_amount.toFixed(2));
                    console.log("all_discounts_applied");
                    ...
                    //            this.do_warn(_t("Error"), _t("Error,Barcode not found"));
                    this.do_warn(_t("Error"), _t("Error,Cannot apply more than 2 discounts. kindly remove or reset the order lines"));
                }
                else {
                    this.all_discounts_applied.push(mode);
            ...
            this.discount_amount = parseFloat(disc + existing_discount).toFixed(2);
            this.set_manual_discount(true);
            //        	this.discount=0;
            //        	this.discountStr = '0';
            //            var existing_discount_amount=
            this.discount_amount_str = '' + this.discount_amount;
            this.trigger('change', this);
        },
        ...
        get_discount_amount_str: function () {
            return this.pos.chrome.format_currency(this.discount_amount_str);
        },
        ...
        set_second_name: function (second_name) {
            this.second_name = second_name;
        },
        get_second_name: function () {
    ...
    models.NumpadState = models.NumpadState.extend({
        appendNewChar: function (newChar) {
            ...
            // Delay the triggering of the 'set_value' event by 500ms
            var self = this;
            setTimeout(function () {
                self.trigger('set_value', self.get('buffer'));
            }, 500);
        },
