...
class PosConfig(models.Model):
	...
	customer_display_details_ids = fields.One2many('customer.display', 'config_ids', "Customer Display Details")
	...
	# kashif 15 april24: added setting for pos login
	enable_pos_login = fields.Boolean(string="Enable Pos login", default=False)
	
	# kashif 30 may24: added setting for pos login
	customer_dislay_login = fields.Boolean(string="Enable Cust. Display as login", default=False)
...
class pos_order(models.Model):
	...
	def test_paid(self):
		...
		if res == False:
			for order in self:
				...
				if float_is_zero(order.amount_total + order.rounding - total_paid,
				                 precision_rounding=3) or float_is_zero(order.amount_total - total_paid,
	...
	def check_order_delivery_type(self):
		if self.delivery_type == 'pending' and self.state == 'draft':
			action_id = self.env.ref('flexiretail_advance.action_pos_payment_flexiretail')
			return {
				'name': action_id.name,
				'type': action_id.type,
	...
	def action_pos_order_paid(self):
		...
		if not self.test_paid():
			...
			# run the sanity check BEFORE allowing the order to go to 'paid'
			self._check_abnormal_statement_amounts()
	...
	@api.multi
	def _check_abnormal_statement_amounts(self):
		"""
        Extra validation/fix to catch insane POS payments like:
        -209,555,418,887.95 / +209,555,418,902.00 / 14.06

        If something looks 'huge', auto-correct:
        - Sum of NON-rounding statement lines -> order.amount_total
        - Rounding lines -> nearest 0.05 (MYR 5-sen) difference
        """
		for order in self:
			lines = order.statement_ids
			if not lines:
				continue
			
			order_amount_abs = abs(order.amount_total) or 1.0
			
			# Only trigger if some line is way bigger than the order itself
			THRESHOLD_FACTOR = 100.0  # tweak if needed
			if not any(abs(l.amount) > THRESHOLD_FACTOR * order_amount_abs for l in lines):
				# looks normal, don't touch it
				continue
			
			currency = order.pricelist_id.currency_id or order.company_id.currency_id
			prec = currency.decimal_places or 2
			
			# --- Identify rounding vs non-rounding lines ---
			rounding_lines = lines.filtered(
				lambda l: l.journal_id and l.journal_id.jr_use_for == 'rounding'
			)
			non_rounding_lines = lines - rounding_lines
			
			# --- Target rounding: nearest 0.05 (MYR 5-sen) ---
			# nearest 0.05: multiply by 20, round, divide back
			target_payable = float_round(order.amount_total * 20.0,
			                             precision_digits=0) / 20.0
			expected_rounding = float_round(
				target_payable - order.amount_total,
				precision_digits=prec,
			)
			# e.g. 14.06 → target 14.05 → expected_rounding = -0.01
			
			# --- Fix NON-rounding lines: make them equal to order.amount_total ---
			if non_rounding_lines:
				# Prefer a positive line as the "main" payment
				pos_lines = non_rounding_lines.filtered(lambda l: l.amount > 0)
				if pos_lines:
					main_pay_line = pos_lines.sorted(key=lambda l: l.amount, reverse=True)[0]
				else:
					# fallback: the line with largest absolute amount
					main_pay_line = non_rounding_lines.sorted(
						key=lambda l: abs(l.amount), reverse=True
					)[0]
				
				# Set main payment line to the order total (before rounding)
				main_pay_line.amount = float_round(order.amount_total,
				                                   precision_digits=prec)
				
				# Zero out the other non-rounding lines (they are the "crazy" ones)
				extra_non_round = (non_rounding_lines - main_pay_line)
				if extra_non_round:
					extra_non_round.write({'amount': 0.0})
			
			# --- Fix rounding lines: set to expected_rounding ---
			total_rounding_fixed = 0.0
			if rounding_lines:
				main_round_line = rounding_lines.sorted(
					key=lambda l: abs(l.amount), reverse=True
				)[0]
				main_round_line.amount = expected_rounding
				total_rounding_fixed = expected_rounding
				
				# Any extra rounding lines -> 0
				extra_rounding = (rounding_lines - main_round_line)
				if extra_rounding:
					extra_rounding.write({'amount': 0.0})
			else:
				# No rounding line but rounding expected: we just log it for now.
				if float_compare(expected_rounding, 0.0, precision_digits=prec) != 0:
					_logger.warning(
						'POS order %s has abnormal statements but no rounding journal line; '
						'expected rounding %.2f',
						order.name, expected_rounding,
					)
			
			# --- Logging after auto-fix ---
			total_non_round_fixed = float_round(
				sum(non_rounding_lines.mapped('amount')),
				precision_digits=prec,
			) if non_rounding_lines else 0.0
			
			_logger.info(
				'Auto-fixed POS statement amounts for order %s: '
				'non_round=%s, rounding=%s (expected rounding=%s, target_payable=%s)',
				order.name,
				total_non_round_fixed,
				total_rounding_fixed,
				expected_rounding,
				target_payable,
			)
	...
	def create_from_ui(self, orders):
		...
		# prevent duplicate order number. assume existing order with paid/posted status is bug name
		paid_pos_order = self.search([('id', 'in', pos_order.ids), ('state', '!=', 'draft')]).mapped(
			'pos_reference') if pos_order else False
		if paid_pos_order:
			order_count = {}
			for i in range(len(orders)):
				order_data = orders[i]['data']
				if order_data['amount_paid'] == 0 and order_data['amount_total'] == 0 and order_data[
					'amount_paid'] == 0:
					continue
				order_name = order_data['name']
				existing_order = self.search([('pos_reference', '=', order_name)], limit=1)
				
				# Determine the starting number for each order name
				if order_name in paid_pos_order and existing_order:
					creation_date = datetime.strptime(orders[i]['data']['creation_date'], '%Y-%m-%dT%H:%M:%S.%fZ')
					# extra validation: only process order if order date (accuracy by milisecond) is different
					if creation_date != existing_order.date_order:
						# Initialize count based on the first duplicate
						order_count[order_name] = order_count.get(order_name, 1) + 1
						# Create a unique name by appending the count
						orders[i]['data']['name'] = f'{order_name} ({order_count[order_name]})'
					else:
						# If the order name is not in saved_order, just add it
						order_count[order_name] = 1
				else:
					# If the order name is not in saved_order, just add it
					order_count[order_name] = 1
		# existing_pos_ref = paid_pos_order.mapped('pos_reference')
		# for o in orders:
		#     current_order_name = o['data']['name']
		#     if current_order_name in existing_pos_ref:
		#         num = 2
		#         while self.search([('pos_reference','=',current_order_name), ('state','!=', 'draft')]):  # If the number is same as existing paid order
		#             current_order_name = current_order_name + f' ({num})' #1. 1 (2). 2
		#             num += 1
		#         while current_order_name in submitted_references:  # If the number is same as existing paid order
		#             current_order_name = current_order_name + f' ({num})'
		#             num += 1
		#         o['data']['name'] = current_order_name
		#     current_submitted_references = [o['data']['name'] for o in orders]
		...
		for tmp_order in orders_to_save:
			...
			if pos_order.is_rounding:
				# pos_order.amount_paid = pos_order.amount_total + pos_order.rounding
				#     use sql to prevent onchange/write function
				query = f"""
                    UPDATE pos_order
                    SET amount_paid = {pos_order.amount_total + pos_order.rounding}
                    WHERE id = {pos_order.id};
                """
				self.env.cr.execute(query)
				self.env.cr.commit()
			try:
				# if draft order, delete all payments, mainly to remove rounding payment
				if order.get('is_draft_order'):
					pos_order.statement_ids.unlink()
				elif pos_order.state == 'draft' and pos_order.statement_ids:
					rounding_journal_id = pos_order.session_id.config_id.rounding_journal_id
					if pos_order.rounding != 0:
						if rounding_journal_id and rounding_journal_id.id not in pos_order.statement_ids.mapped(
								'journal_id').ids:
							# add back rounding payment
	...
	def _process_order(self, order):
		...
		if order.get('old_order_id'):
			# validation to prevent editing of paid order
			# Search for the draft order with the specified old_order_id
			draft_order_with_statement = self.env['pos.order'].search([
				('id', '=', order.get('old_order_id')),
				('state', '=', 'draft')
			], limit=1)  # Use limit=1 to ensure only one record is returned
			
			draft_order = False
			
			if draft_order_with_statement:
				# Check if the draft order has no statement_ids
				if not draft_order_with_statement.statement_ids:
					draft_order = draft_order_with_statement
				else:
					# If statement_ids exist, check rounding journal conditions
					config = draft_order_with_statement.session_id.config_id
					if config and config.enable_rounding:
						rounding_journal = config.rounding_journal_id
						# Check if all statement journals match the rounding journal
						if all(stmt.journal_id == rounding_journal for stmt in
						       draft_order_with_statement.statement_ids):
							draft_order = draft_order_with_statement
			if not draft_order:
				order.pop('old_order_id')
			else:
		...
		if order.get('draft_order'):  # first we make draft order
			if not draft_order_id:
				...
				res = order_id
		if not order.get('draft_order') and draft_order_id:  # posted/paying existing draft order
			order_id = draft_order_id
			order_obj = self.browse(order_id)
			self._cr.execute(
				"UPDATE pos_order set session_id=%s where id = %s" % (order.get('pos_session_id'), order_id))
			pos_line_ids = pos_line_obj.search([('order_id', '=', order_id)])
			if pos_line_ids:
			...
			if order.get('is_rounding') and order.get('rounding'):
				order_obj.statement_ids
		...
		if not order.get('draft_order') and not draft_order_id:
			# check if this order has been made
			order_data = self._order_fields(order)
			existing_order = self.search([
				('session_id', '=', order_data['session_id']),
				('pos_reference', '=', order_data['pos_reference']),
				...
				('amount_return', '=', order_data['amount_return']),
			])
			
			if not existing_order:
				res = super(pos_order, self)._process_order(order)
			else:
				# do not create another order if one with same values exist
				res = existing_order
			
			if res:
				# Detect and correct payment inconsistencies (e.g. credit card: 2,000,000 and change: 1,999,900) → adjust credit card amount to 100 and remove change line
				big_lines = res.statement_ids.filtered(lambda l: abs(l.amount) > 100000)
				if big_lines:
					res.readjust_statements()
			...
			if loyalty_settings and loyalty_settings[0]:
				if loyalty_settings[0].get('points_based_on') and order.get('loyalty_earned_point'):
					...
					if loyalty and res.partner_id.send_loyalty_mail:
						try:
							template_id = self.env['ir.model.data'].get_object_reference('flexiretail_advance',
		...
		# make sure name of payment lines is same as order name
		if res.lines:
			res.lines.write({'name': res.name})
		return res
	
	def readjust_statements(self):
		for rec in self:
			big_lines = rec.statement_ids.filtered(lambda l: l.amount > 1000000)
			if big_lines:
				total_amount = 0
				positive_statement_id = False
				negative_statement_id = False
				
				# calculate total amount, and find positive/negative statement line to use
				for statement in rec.statement_ids:
					amount = statement.amount
					total_amount += amount
					if round(amount, 2) > 0:
						positive_statement_id = statement
					else:
						negative_statement_id = statement
				# remove other statement id
				if total_amount > 0:
					id_to_remove = rec.statement_ids.ids.remove(positive_statement_id.id)
					statement_id = positive_statement_id
				else:
					id_to_remove = rec.statement_ids.ids.remove(negative_statement_id.id)
					statement_id = negative_statement_id
				self.env['account.bank.statement.line'].browse(id_to_remove).unlink()
				statement_id.amount = total_amount
	...
	def _compute_amount_due(self):
		for each in self:
			...
			if each.is_rounding:
				each.amount_due += each.rounding
			each.amount_due = round(each.amount_due, 2)
				...
				if ('category_summary_report' in vals['summary'] or len(vals['summary']) == 0):
					...
					for each_order in orders:
						for order_line in each_order.lines:
							...
							# update count d.p.
							try:
								# amount = round(amount, self.env.user.company_id.currency_id.decimal_places)
								count = round(count, self.env.ref('product.decimal_product_uom').digits)
							except:
								pass
	...
	rounding = fields.Float(string='Rounding', digits=(12, 3))
...
class pos_order_line(models.Model):
	...
	def _compute_amount_line_all(self):
		res = super(pos_order_line, self)._compute_amount_line_all()
		...
		for line in self:
			...
			# Step 2: remainder check (safe float handling)
			diff = float_round(subtotal - base, precision_rounding=0.01)
			
			# Step 3: apply 5-cent rounding rules
			if diff >= 0.08:
				base = float_round(base + 0.10, precision_rounding=0.05)
			elif diff >= 0.03:
				base = float_round(base + 0.05, precision_rounding=0.05)
...
class pos_session(models.Model):
	...
	def create(self, vals):
		...
		if res:
			# kenny 29/05/2024 - set balance start for rounding to 0
			for statement in res.statement_ids:
				if statement.journal_id.jr_use_for == 'rounding' or statement.journal_id.type == 'cash':
					statement.balance_start = 0
			# kenny 29/05/2024 - set balance start for rounding to 0
			
			context = dict(self._context)
			balance_type = context.get('balance') or 'start'
			# context['bank_statement_id'] = res.cash_register_id.id
			# context['balance'] = balance_type
			# context['default_pos_id'] = res.config_id.id
			...
			#     cashbox_id = res.cash_register_id.cashbox_start_id.id
			# else:
			#     cashbox_id = self.cash_register_id.cashbox_end_id.id
			# if cashbox_id:
			#     action['res_id'] = cashbox_id
			
			statement = self.env['account.bank.statement.cashbox'].with_context(
				bank_statement_id=res.cash_register_id.id, balance=balance_type,
				default_pos_id=res.config_id.id).create({})
			if statement:
				statement.validate_custom()
		return res
	...
	def custom_close_pos_session(self):
		...
		self.write(
			{'state': 'closing_control', 'stop_at': fields.Datetime.now()})  # make sure closing date is not empty
		return self.with_context(closing_pos=True).action_pos_session_close()
		# for session in self:
		#     if not session.config_id.cash_control:
		#         session.with_context(closing_pos=True).action_pos_session_close()
		#         return True
	...
	def _check_amount(self):
		if not self._context.get('from_pos'):
			super(pos_session, self)._check_amount()
	
	@api.one
	@api.constrains('amount', 'amount_currency')
	def _check_amount_currency(self):
		if not self._context.get('from_pos'):
			super(pos_session, self)._check_amount_currency()
	
	def action_pos_session_close(self):
		'''
        before closing POS session starts, check if any draft order should be in paid status
        '''
		if self:
			pos_orders = self.env['pos.order'].search([('session_id', 'in', self.ids), ('state', '=', 'draft')])
