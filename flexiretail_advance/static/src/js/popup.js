odoo.define('flexiretail_advance.popup', function (require) {
    "use strict";
                    ...
                    rpc.query(params, {async: false}).then(function(res){
                        if(res){
                            ...
                            if(self.pos.config.z_report){
                                ...
                                self.pos.chrome.do_action('flexiretail_advance.pos_z_report',{
                                    additional_context:{
                                ...
                                if(self.pos.config.iface_print_via_proxy){
                                    var report_name = "flexiretail_advance.pos_z_thermal_report_template";
                                    var params = {
                            ...
                            if(result_find.can_give_discount){
                               if( this.mode === 'discount'){
                                ...
                                alert(_t(result_find.name + ' does not have right to give discount.'));
                                      ...
                                      if(result_find.can_update_price){
                                self.pos.get_order().get_selected_orderline().set_unit_price(self.value);
                                self.pos.get_order().get_selected_orderline().price_manually_set = true;

                                this.gui.close_popup();
                            } else {
                                alert(_t(result_find.name + ' does not have right to update price.'));
                            }
                        }
                        }
                        else {
...
//    		var session_id = session.session_id
                        if(self.pos.config.customer_dislay_logout){
                          window.open('https://www.excelroot.com/?post_url='+self.pos.attributes.origin+'/web/customer_display&session_id=close' , '_blank');
                          }
                      self.pos.gui.close();
//            });
...
//	Multi-Store Popup
    ...
    var ReportPopupWidget = PopupWidget.extend({
        ...
        session_report_pdf: function(e){
            ...
            self.pos.chrome.do_action('flexiretail_advance.report_pos_inventory_session_pdf_front',{additional_context:{
        ...
        session_report_thermal: function(e){
            var self = this;
            var session_id = $(e.currentTarget).data('id');
            var report_name = "flexiretail_advance.front_inventory_session_thermal_report_template";
            var params = {
                model: 'ir.actions.report',
                method: 'get_html_report',
        ...
        location_report_pdf: function(e){
            ...
            self.pos.chrome.do_action('flexiretail_advance.report_pos_inventory_location_pdf_front',{additional_context:{
        ...
        location_report_thermal: function(e){
            var self = this;
            var location_id = $(e.currentTarget).data('id');
            var report_name = "flexiretail_advance.front_inventory_location_thermal_report_template";
            var params = {
                model: 'ir.actions.report',
                method: 'get_html_report',
         ...
         click_confirm: function(){
             ...
             rpc.query(params, {async: false}).then(function(vals){
                 if(vals){
                    ...
                    if (self.pos.config.iface_print_via_proxy) {
                        var receipt = QWeb.render('AddedCreditXmlReceipt',{
                            widget: self,
                            order: order,
        ...
        click_confirm: function(){
            ...
            rpc.query(params, {async: false}).then(function(vals){
                if(vals){
                    ...
                    if (self.pos.config.iface_print_via_proxy) {
                        var receipt = QWeb.render('PayDebitXmlReceipt',{
                            widget: self,
                            order: order,
                            get_journal_id: get_journal_id,
                            amount: amount,
