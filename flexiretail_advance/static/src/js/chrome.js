odoo.define('flexiretail_advance.chrome', function (require) {
"use strict";
...
//		},
		...
		build_widgets: function(){
			...
			if (partner && self.pos.get_cashier() && self.pos.get_cashier().access_default_customer) {
			    if (partner[0]){
//			        11.1.19 Kenny - add delay to let partner data loads first
			        setTimeout(function() {
                    var set_partner = self.pos.db.get_partner_by_id(partner[0]);
                    console.log('set_partner', set_partner)
                    if (set_partner) {
		...
		save_receipt_for_reprint:function(){
            ...
            var receipt_html = QWeb.render('PosTicket_custom_salesperson',env);
...
//					self.pos.db.notification('success',_t(notif[1].sync_order_paid[0].display_name) + 'Order has been paid');
		...
		render_sale_note_order_list: function(orders){
        	var self = this;
        	if (this.pos.config.has_sticker){
        	        	if(orders){
...
//                });
        	...
        	self.x_report = function(){
        		...
        		self.pos.chrome.do_action('flexiretail_advance.pos_x_report',{additional_context:{
                    active_ids:pos_session_id,
                }})
//                always output fail even if it succeeds
//                .fail(function(){
//                	self.pos.db.notification('danger',"Connection lost");
//                });
        		if(self.pos.config.iface_print_via_proxy){
                    var pos_session_id = self.pos.pos_session.id;
                    var report_name = "flexiretail_advance.pos_x_thermal_report_template";
                    var params = {
...
//        	}
        	...
        	$('body').on('click', '.main_slider-ul', function() {
        	    $('ul.content-list-ul-dash').slideToggle();
        	});
        		$('.main_slider-ul-main').click(function() {
        	    $(this).find('ul.content-list-ul').slideToggle();
        	});
        	$('body').on('click', '.main_slider-ul-main', function() {
        	    $('ul.content-list-ul-repo').slideToggle();
