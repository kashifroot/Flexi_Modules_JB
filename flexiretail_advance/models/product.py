...
class ProductTemplate(models.Model):
	...
	_order = 'pos_sequence asc'
	
	total_sold_qty = fields.Float(
		string='Total Sold Quantity (POS)',
		compute='_compute_total_sold_qty',
		store=False,
		help='Total quantity sold through POS orders.'
	)
	...
	def _compute_total_sold_qty(self):
		...
		for product in self.search([('available_in_pos', '=', True)]):
			# Get all POS order lines for this product where order is done/paid/invoiced
			lines = pos_line.search([
...
class ProductProduct(models.Model):
	...
	@api.model
	def load_latest_product_price(self, write_date, fields, context):
		# Fetch only products modified after the provided write_date
		variant_ids = self.with_context(context).search([
			("sale_ok", "=", True),
		...
		tmpl_ids = list({x.product_tmpl_id.id for x in variant_ids})  # Remove duplicates with set comprehension
		
		# Fetch the templates that have been updated
		product_ids = self.env['product.template'].search([('id', 'in', tmpl_ids)])
		
		# Read only the specified fields for variants
		variants_data = variant_ids.with_context(context).read(fields)
		tmpl_data = product_ids.read(['id', 'name', 'list_price'])  # Adjust fields as needed for template data
		
		return {
			'product_tmpl': tmpl_data,
	...
	def expiry_product_alert(self):
		...
		if notification_days:
			...
			email_list = []
			template_id = self.env.ref('flexiretail_advance.email_template_product_expiry_alert')
