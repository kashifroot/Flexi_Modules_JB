...
class Home(Home):
    ...
    @http.route('/hw_proxy/open_cashbox_custom', type='json', auth='none', cors='*',sitemap=False,csrf=False)
    def open_cashbox(self):
        _logger.info('ESC/POS: OPEN CASHBOX')
        device = serial.Serial(port="/dev/ttyS0", baudrate=9600, bytesize=8,
                                    parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=1,
                                    dsrdtr=True)
        msg = "" + (str)(27) + (str)(112) + (str)(0) + (str)(60) + (str)(255);
        device.write(msg)
...
class DataSet(http.Controller):
    ...
    def send_pos_ordermail(self, **kw):
        ...
        for order in orders:
            if order and order.reserved and order.session_id and order.session_id.config_id.enable_pos_welcome_mail:
                try:
                    template_id = request.env['ir.model.data'].get_object_reference('flexiretail_advance',
    ...
    def load_customers(self, **kw):
        ...
        if eval(kw.get('fields')):
            fields = eval(kw.get('fields'))
        domain = [('id', 'in', partner_ids), ('customer', '=', True),('active','=',True)]
        try:
            records = request.env['res.partner'].search_read(domain, fields,limit=50)
            if records:
                for each_rec in records:
...
class PosMirrorController(http.Controller):
    ...
    def white_board_web(self, **k):
        ...
        return request.render('flexiretail_advance.customer_display_index', qcontext=context)
