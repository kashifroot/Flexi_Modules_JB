...
class ResUsers(models.Model):
    ...
    can_update_price = fields.Boolean("Update price", default=True)
    ...
    open_session_during_delivery = fields.Boolean(string="Can Open Session During Delivery")
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        if self._context.get('from_sales_person'):
            ...
            # kashif 6 may 24 : fixed salesperon issue
            sale_person_ids = self.search([('id', 'in', pos_users_ids),('id','!=',self._context.get('uid')),
            ...
            if sale_person_ids:
                # print('sale_person_ids.ids ', sale_person_ids.ids)
                # users.append(sale_person_ids.ids)
                users.extend(sale_person_ids.ids)
            if users:
                args = args or []
                args += [['id', 'in', users]]
