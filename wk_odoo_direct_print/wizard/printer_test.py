...
class WizardTestPrinter(models.TransientModel):
    ...
    def test_printer(self):
        ...
        for each_partner in printer_id.hostmachine_id.user_ids:
            message = {'type': 'print_direct',
                       'payload': {'method': 'print-job-cmd', 'host_id': printer_id.hostmachine_id.host_id, 'record_id': new_print_job.id, }}
            self.env['bus.bus'].sendone(
                (self._cr.dbname, 'res.partner', each_partner.partner_id.id), message)


        # print(f'\n\n\n=====>>{self.env.user.search([])} test_printer{self.env.user.partner_id}')
        # # for each_partner in self.env.user.search([]):
        # for each_partner in printer_id.hostmachine_id.user_ids:
        #     data = [each_partner.partner_id, 'mail.record/insert', {'Persona': [{
        #         'id': 121212, 
        #         'im_status': 'print direct', 
        #         'type': 'partner', 
        #         'data':{'method': 'print-raw', 'host_id':printer_id.hostmachine_id.host_id, 'print_data': print_data}
        #         }]}]
        #     self.env['bus.bus']._sendmany([data])
