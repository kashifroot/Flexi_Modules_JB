odoo.define('flexiretail_advance.screens', function (require) {
	"use strict";
	...
	var flexi_chrome = require('flexiretail_advance.chrome')
				...
				$.ajax({
					...
					success: function (res) {
						...
						if (cache_products && cache_products.length > 0) {
							...
							_.map(cache_products, function (product) {
								product_ids_list.push(product.id)
							});
							...
							$('#product_sync').hide();
							if (product_ids_list && product_ids_list.length > 0) {
								var product_ids = product_ids_list;
								...
								if (total_products) {
									...
									function ajax_product_load() {
										if (count_loop > 0 && last_ids.length > 0) {
											var product_fields = self.pos.product_fields.concat(['name', 'display_name', 'product_variant_ids', 'product_variant_count'])
											$.ajax({
												type: "POST",
												...
												success: function (res) {
													...
													if (count_loaded_products >= total_products) {
														...
														$('.pos.order-container.order').addClass('collasped_subwindow');
	...
	var LoginScreenWidget = screens.ScreenWidget.extend({
		...
		start: function () {
			...
			$('input#password, input#username').keypress(function (e) {
				if (e.keyCode == 13) {
					...
					//			        kenny - 14/09/2023 enhancement remove pin checking if pin not set up
		...
		login_user: function (username, password) {
			...
			var user = _.find(self.pos.users, function (obj) {
				return (obj.login == username && obj.pos_security_pin == password) || obj.pos_security_pin === false;
			});
			//                    end - kenny
			if (user) {
				$('.pos-topheader').show();
							...
							if (count_loop > 0) {
								$.ajax({
									...
									success: function (res) {
										...
										if (self.pos.config.has_sticker == true) {
											count_loop = 0;
										}
										else {
										...
										if (all_partners.length) {
											console.log('next customer list loaded')
											ajax_partner_load();
										}
										else {
											console.log('no customer found, ' + retries + ' retries left');
											--retries;
											if (retries >= 0) {
												ajax_partner_load();
											}
	...
	screens.PaymentScreenWidget.include({
		...
		click_edit_order: function () {
			...
			if (order) {
				...
				// Fetch latest metadata from backend using RPC
				var pos_ref = order.get_pos_reference() || order.name;
				rpc.query({
					model: 'pos.order',
					method: 'search_read',
					...
					if (results && results.length > 0) {
						...
						if (backend_order.salesman_id) {
							order.set_salesman_id(backend_order.salesman_id[0]);
							// Sync with custom_jadi_tour salesperson_id if present
							if (order.hasOwnProperty('salesperson_id')) {
								order.salesperson_id = backend_order.salesman_id[0];
								order.salesperson_name = backend_order.salesman_id[1];
								order.set_salesperson_name(backend_order.salesman_id[1]);
							}
						}
						...
						if (self.pos.chrome.screens.products) {
							self.pos.chrome.screens.products.show();
						}
					}
				});
			}
		...
		click_numpad: function (button) {
			...
			if (!open_paymentline) {
				this.pos.get_order().add_paymentline(
					_.filter(this.pos.cashregisters, function (item) {
						return item.journal_id[1].includes('Cash') || item.journal_id[1].includes('cash');
					})[0]);
		...
		finalize_validation: function () {
			...
			if (partner && (order.get_remaining_credit() || order.get_remaining_credit() == 0)) {
				if (order.get_remaining_credit() >= 0) {
					partner.remaining_credit_amount = order.get_remaining_credit();
					if (partner.remaining_credit_amount !== false) {
						var params = {
							model: 'res.partner',
							method: 'write',
							args: [partner.id, { 'remaining_credit_amount': partner.remaining_credit_amount || 0 }],
						}
						rpc.query(params, { async: false })
							.then(function (res) {
							});
		...
		click_back: function () {
			var self = this;
			var order = this.pos.get_order();
			if (order && order.get_credit_detail() && order.get_credit_detail().length > 0) {
	...
	screens.OrderWidget.include({
		...
		render_orderline: function (orderline) {
			var scroll_order = false;
			var scroll = 0.0;
			$('.orderline').each(function () {
				let orderlineHeight = $(this).outerHeight();
				let productName = $(this).find('.product-name').text().trim();
				scroll = scroll + orderlineHeight;
				if (productName === orderline.product.name || productName === orderline.product.display_name) {
					let unitText = parseFloat($(this).find('.info-list .info em').text().trim());
					let newUnit = parseFloat(orderline.quantity)
					if (unitText !== newUnit) {
						scroll_order = true;
						scroll = scroll - orderlineHeight;
						return false
			...
			let orderScroller = $(".order-scroller .order");
			if (scroll_order === true) {
				orderScroller.animate({ scrollTop: scroll }, 50);
			}
			else {
				orderScroller.scrollTop($('.order-scroller .order').height() * 15);
			}
					...
					if (!selected_line.get_promotion()) {
						if (this.pos.config.enable_operation_restrict) {
							if (order.get_selected_orderline()) {
								...
								if (mode === 'quantity') {
								...
								} else if (mode === 'discount' || mode === 'discountAmnt') {
									if (cashier && cashier.can_give_discount) {
										if (val <= cashier.discount_limit || cashier.discount_limit < 1) {
											if (mode === 'discount') {
												...
												order.get_selected_orderline().set_discount_amount(val);
											}
											if (val == '') {
											...
											if (cashier.based_on == 'barcode') {
												this.gui.show_popup('ManagerAuthenticationPopup', { val: val, mode: mode });
											}
											else {
												...
												var res = self.gui.authentication_pin(password).then(function () {
													if (mode === 'discount') {
														...
														self.pos.get_order().get_selected_orderline().set_discount_amount(val);
													}
													self.gui.close_popup();
												});
											}
									...
									if (cashier && cashier.can_update_price) {
										if (val == '') {
											this.numpad_state.change_mode = true
										}
										else {
											if (cashier.based_on == 'barcode') {
												...
												var res = self.gui.authentication_pin(password).then(function () {
													...
													self.pos.get_order().get_selected_orderline().set_unit_price(val);
													self.pos.get_order().get_selected_orderline().price_manually_set = true;
													self.gui.close_popup();
												});
											}
									...
									else if (mode === 'discountAmnt') {
										if (val.slice(-1) !== '.' & val !== "0") {
											order.get_selected_orderline().set_discount_amount(val, mode);
										}
										if (val == '') {
											this.numpad_state.change_mode = true
										}
										//					            			yulia
										else if (mode === 'discountTotal') {
											if (val.slice(-1) !== '.' & val !== "0") {
												order.set_discount_total(order.get_orderlines(), val, mode)
											}
											if (val == '') {
												this.numpad_state.change_mode = true
											}
										}
										else if (mode === 'discountTotalPercent') {
											if (val.slice(-1) !== '.' & val !== "0") {
												order.set_discount_total_percent(order.get_orderlines(), val, mode)
											}
											if (val == '') {
												this.numpad_state.change_mode = true
											}
											...
											order.reset_orderlines(order.get_orderlines(), val)
											if (val == '') {
												this.numpad_state.change_mode = true
											}
										}
										...
										self.pos.db.notification('danger', _t('You don\'t have access to update price.'));
									}
								}

							}
						}
						else if (!this.pos.config.enable_operation_restrict) {
							if (order.get_selected_orderline()) {
								var mode = this.numpad_state.get('mode');
								if (mode === 'discountAmnt') {
									if (val.slice(-1) !== '.' & val !== "0") {
										order.get_selected_orderline().set_discount_amount(val, mode);
									}
									if (val == '') {
	...
	screens.ProductScreenWidget.include({
		start: function () {
			...
			//    		var def = this._rpc({route: '/web/session/get_session_info'}).then(function (session) {
			//    		var session_id = session.session_id
			//      window.open('https://www.excelroot.com/?post_url='+self.pos.attributes.origin+'/web/customer_display&session_id='+session_id , '_blank');
			//            });
			...
			//$('div#draft_order').on('click', function() {
			//    console.log('clllllicked')
			//});
			$('body').on('click', '#draft_order', function () {
				if (self.pos.get_order().get_sale_mode()) {
					self.create_draft_order();
				} else {
					self.pos.db.notification('danger', _t('Sorry, This operation only allows Sale Mode!'));
				}
			});
			...
			$('body').on('click', '#multi-shop', function () {
			...
			$('body').on('click', '#customer_display', function () {
			...
			//		    	}
			//			});
			$('body').on('click', '#delivery_mode', function () {
			...
			$('body').on('click', '#money_in', function () {
			...
			$('body').on('click', '#money_out', function () {
			...
			$('body').on('click', '#add_credit', function () {
			...
			$('body').on('click', '#pay_debit', function () {
			...
			$('body').on('click', '#discard_product', function () {
			...
			$('body').on('click', '#set_customer_button', function () {
				self.gui.show_screen('clientlist');
				$('.menu_toggle').click();
			});
		...
		create_draft_order: function () {
			...
			//            get_total_with_tax is default odoo function and doesn't consider rounding adjustment. getNetTotalTaxIncluded is a function from flexi
			//        	if(order.get_due().toFixed(2) != order.get_total_with_tax().toFixed(2)){
			if (order.get_due().toFixed(2) != order.getNetTotalTaxIncluded().toFixed(2)) {
		...
		show: function () {
			...
			if (this.pos.get_order().get_sticker()) {
				$('.sticker-text').text(this.pos.get_order().get_sticker());
			} else {
	...
	screens.ReceiptScreenWidget.include({
		...
		print_web: function () {
			if ($.browser.safari) {
				...
				//            $('.pos-rightheader').empty();
				$('.pos-rightheader').attr('style', 'display: none');
				...
				//           $('.pos-rightheader').html(header_html);
		...
		render_receipt: function () {
			...
			if (order.get_free_data()) {
				...
				if (order && order.is_reprint) {
					this.$('.total-value').html(this.format_currency(order.getNetTotalTaxIncluded() || order.amount_paid));
					this.$('.change-value').html(this.format_currency(order.getNetTotalTaxIncluded() || order.amount_return));
...
//						});
			...
			if (barcode_val.indexOf(_t("Order ")) != -1) {
				...
				if (vals) {
					...
					$("tr#barcode2").html($("<td style='padding:2px 2px 2px 0px; text-align:center;'><div class='" + barcode + "' width='300' height='400'/></td>"));
					$("." + barcode.toString()).barcode(barcode.toString(), "code128", {
						barWidth: 2, // Width of each bar in pixels (default is usually 1)
						barHeight: 100  // Overall height of the barcode
					});
				}
			}
		},
	...
	var OrderListScreenWidget = screens.ScreenWidget.extend({
		...
		click_reprint: function (event) {
			...
			if (result.pos_normal_receipt_html) {
				...
				selectedOrder.amount_return = result.amount_return;
				selectedOrder.amount_paid = result.amount_paid;
		...
		click_edit_order: function (event) {
			...
			if (result.lines.length > 0) {
				...
				//	            if (selectedOrder.attributes.draft_order){
				$('.order-information').removeClass('oe_hidden');
				selectedOrder.set_draft_order(true);
				$('.order-information').removeClass('oe_hidden');
				//	            }
				self.gui.show_screen('products');
				if (delivery_mode) {
		...
		render_list: function (orders) {
			...
			if (orders) {
				...
				for (var i = 0, len = Math.min(orders.length, 1000); i < len; i++) {
					...
					// Get the data-id of the clientline
					var dataId = clientline.getAttribute('data-id');

					// Check if tr with the same data-id exists in contents
					if (!contents.querySelector('tr[data-id="' + dataId + '"]')) {
						contents.appendChild(clientline);
					}
					//                    else {
					//                        console.warn('Duplicate data-id found: ' + dataId + '. Skipping insertion.');
					//                    }
				}
			}
		},
	...
	screens.NumpadWidget.include({
		start: function () {
			...
			this.$el.find('.numpad-reset').click(_.bind(this.clickNumpadReset, this));

		},
		clickNumpadReset: function () {
			self.pos.get_order().reset_orderlines()
		},
	});
	...
	screens.ActionpadWidget.include({
		renderElement: function () {
			...
			this.$('.pay').unbind('click').click(function () {
				...
				var elem = $('.sticker-text');
				if (order.get_sticker() === false && elem.length > 0) {
					self.gui.show_screen('products');
					//                                       self.pos.db.notification('danger',_t('Please enter valid sticker.'));
					self.pos.gui.show_popup('error', {
						'title': _t('Invalid Sticker'),
						'body': _t('Please check if the correct sticker is selected.'),
					});
					return;
				}
			...
			this.$('.set_customer').click(function () {
				self.gui.show_screen('clientlist');
			});
		},
		//        set customer function added
		partner_icon_url: function (id) {
			return '/web/image?model=res.partner&id=' + id + '&field=image_small';
		},
	});
					...
					if (self.pos.config.paid_amount_product) {
						...
						if (result.invoice_ids.length > 0) {
							var params = {
								model: 'account.invoice',
								method: 'search_read',
								domain: [['id', 'in', result.invoice_ids], ['state', 'in', ['draft']]]
							}
