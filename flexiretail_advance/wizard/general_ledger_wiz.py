...
class account_general_ledger_wiz(models.TransientModel):
    ...
    def print_pdf(self, data):
        ...
        return self.env.ref('flexiretail_advance.report_general_ledger').report_action(self, data=datas)
    ...
    def print_xls(self, data):
        ...
        report_obj = self.env['report.flexiretail_advance.general_ledger_template']
        data = {}
        data['ids'] = self.env.context.get('active_ids', [])
        data['model'] = self.env.context.get('active_model', 'ir.ui.menu')
        data['form'] = self.read(
